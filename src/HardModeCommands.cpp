...
bool HardModeCommandsScript::HandleHardModeInfoCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
{
    ...
    handler->SendSysMessage(Acore::StringFormat("启用的难度模式: {}", sHardModeHandler->GetNamesFromEnabledModes(targetPlayer)));
    handler->SendSysMessage(Acore::StringFormat("IsTainted: {}", sHardModeHandler->IsPlayerTainted(targetPlayer->GetGUID())));
    handler->SendSysMessage(Acore::StringFormat("IsShadowBanned: {}", sHardModeHandler->IsPlayerShadowBanned(targetPlayer->GetGUID())));
...
bool HardModeCommandsScript::HandleHardModeSetTaintCommand(ChatHandler* handler, Optional<PlayerIdentifier> target, uint8 value)
{
    ...
    handler->SendSysMessage(Acore::StringFormat("Updated taint for player '{}' to '{}'.", targetPlayer->GetName(), value));
...
bool HardModeCommandsScript::HandleHardModeSetShadowBanCommand(ChatHandler* handler, Optional<PlayerIdentifier> target, uint8 value)
{
    ...
    handler->SendSysMessage(Acore::StringFormat("Updated shadow ban for player '{}' to '{}'.", targetPlayer->GetName(), value));
